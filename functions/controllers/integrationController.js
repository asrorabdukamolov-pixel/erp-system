...
async function checkAmoLeadsAndTasksInternal() {
    try {
        ...
        // 1. Auto-close leads in 'yangi' (Mijoz ko'tarmadi) stage if older than 168 hours
        try {
            console.log("Checking for stale leads in 'yangi' (Mijoz ko'tarmadi) stage...");
            ...
            if (!staleOrdersSnapshot.empty) {
                ...
                for (const order of staleOrders) {
                    ...
                    if (ageMs >= 168 * 60 * 60 * 1000) {
                        console.log(`Auto-closing order ${order.id} (stale in 'yangi' for > 168 hours)...`);
        ...
        for (const order of orders) {
            ...
            } else if (meetingFormat === 'measurement') {
                if (activeTasks.length === 0) {
