  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (selected) {
        ...
        setCustomerSearch(`${selected.firstName} ${selected.lastName}`);
        setSuggestions([]);
      }
...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* Table Design */
    table { width: 100%; border-collapse: separate; border-spacing: 0; margin-bottom: 40px; border: 1.5px solid #999; border-radius: 16px; overflow: hidden; }
    th { 
      background: #e0e0e0;
      padding: 16px 14px; 
      text-align: left; 
    ...
    td { padding: 20px 14px; border-bottom: 1px solid #d0d0d0; font-size: 14px; vertical-align: middle; }
    tr:last-child td { border-bottom: none; }
    
    .item-img { width: 80px; height: 80px; object-fit: cover; border-radius: 12px; border: 1.5px solid #999; }
...
<body>
  ...
  <div class="info-grid">
    <div class="info-card">
      ...
      <div class="person-name">${selectedCustomer ? `${selectedCustomer.firstName} ${selectedCustomer.lastName}` : '—'}</div>
      <div class="person-details">
        <div>Tel: <b>${selectedCustomer?.phone || '—'}</b></div>
        <div>Manzil: <b>${selectedCustomer?.address || '—'}${selectedCustomer?.houseNumber ? `, ${selectedCustomer.houseNumber}-uy` : ''}${selectedCustomer?.apartmentNumber ? `, ${selectedCustomer.apartmentNumber}-xonadon` : ''}</b></div>
      </div>
    </div>
                    ...
                    <div style={{ position:'absolute', top:'100%', left:0, width:'100%', background:'#1a1a2e', zIndex:100, borderRadius:'12px', border:'1px solid rgba(255,255,255,0.1)', marginTop:'6px', overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.7)' }}>
                      {customerSuggestions.map((c, idx) => (
                        <div key={c._id}
                          ...
                          <span style={{ fontWeight:'700', color: idx === selectedSuggestionIndex ? 'var(--accent-gold)' : 'white' }}>{c.firstName} {c.lastName}</span>
                          <span style={{ color: idx === selectedSuggestionIndex ? 'white' : 'var(--text-secondary)' }}>{c.phone}</span>
                        </div>
                      ))}
                    </div>
                  )}
                      ...
                      {opt.id === 'custom' && deadlineBasis === 'custom' && (
                        <div style={{ paddingLeft: '30px', paddingBottom: '8px' }}>
                           <input 
                             ...
                             placeholder="O'z variantingizni bu yerga yozing..."
                             autoFocus
                             style={{ 
                               width: '100%', 
