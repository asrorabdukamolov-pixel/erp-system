...
<body>
  ...
  <div class="info-grid">
    <div class="info-card">
      ...
      <div class="person-detail">Tel: <b>${kp.customer?.phone || '—'}</b></div>
      <div class="person-detail">Manzil: <b>${kp.customer?.address || '—'}${kp.customer?.houseNumber ? `, ${kp.customer.houseNumber}-uy` : ''}${kp.customer?.apartmentNumber ? `, ${kp.customer.apartmentNumber}-xonadon` : ''}</b></div>
    </div>
  ...
  <div class="summary-grid">
    <div class="deadline-card">
      ...
      <div>
        <span class="deadline-num">${kp.deadline || '—'}</span>
  ...
  const filteredProposals = proposals.filter(p => {
    ...
    const matchesManager = (user.role === 'sotuv_manager' || user.role === 'sales_manager' || user.role === 'proekt_manager') ? p.managerId === (user.id || user._id) : true;
    ...
    <div style={{ padding: '40px', flex: 1, overflowY: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Barcha saqlangan tijorat takliflari va ularning holati.</p>
        </div>
